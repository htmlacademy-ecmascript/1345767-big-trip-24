...
const getFormButtons = (isNewPoint, isSaving, isDeleting) => {
  if (isNewPoint) {
    return `<button class="event__save-btn  btn  btn--blue" type="submit" ${isDeleting ? 'disabled' : ''}>${isSaving ? 'Saving...' : 'Save'}</button>
      <button class="event__reset-btn" type="reset" ${isSaving ? 'disabled' : ''}>${isDeleting ? 'Canceling...' : 'Cancel'}</button>`;
  }
  return `<button class="event__save-btn  btn  btn--blue" type="submit" ${isDeleting ? 'disabled' : ''}>${isSaving ? 'Saving...' : 'Save'}</button>
  ...
  <form class="event event--edit" action="#" method="post">
    <header class="event__header">
      ...
      <div class="event__field-group  event__field-group--destination">
        ...
        <input class="event__input  event__input--destination" id="event-destination-1" type="text" name="event-destination" value="${destinationPoint ? destinationPoint.name : ''}" list="destination-list-1" ${isDisabled ? 'disabled' : ''} required>
      ...
      <div class="event__field-group  event__field-group--time">
        ...
        <input class="event__input  event__input--time" id="event-start-time-1" type="text" name="event-start-time" value="${humanizePointDate(dateFrom, DATE_WITH_TIME_FORMAT)}" ${isDisabled ? 'disabled' : ''} required>
        &mdash;
        <label class="visually-hidden" for="event-end-time-1">To</label>
        <input class="event__input  event__input--time" id="event-end-time-1" type="text" name="event-end-time" value="${humanizePointDate(dateTo, DATE_WITH_TIME_FORMAT)}" ${isDisabled ? 'disabled' : ''} required>
      </div>
      ...
      ${getFormButtons(isNewPoint, isSaving, isDeleting)}
